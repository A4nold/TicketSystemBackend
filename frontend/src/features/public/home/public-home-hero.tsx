...
export function PublicHomeHero({
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-border bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(223,240,242,0.78),rgba(255,236,230,0.82))] px-6 py-8 shadow-[0_24px_70px_rgba(16,32,51,0.14)] sm:px-8 sm:py-10">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(15,143,134,0.16),transparent_32%),radial-gradient(circle_at_bottom_left,rgba(217,95,67,0.14),transparent_28%)]" />
      <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1.25fr)_minmax(18rem,0.85fr)] lg:items-end">
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              href="/marketplace"
              className="rounded-full border border-border bg-surface-elevated px-5 py-3 text-sm font-semibold text-foreground transition hover:bg-surface-soft"
            >
          ...
          <div className="flex flex-wrap gap-2">
            {proofPoints.map((point) => (
              <span
                key={point}
                className="rounded-full border border-border bg-black/15 px-3 py-1.5 text-sm text-foreground"
              >
                {point}
              </span>
            ))}
        ...
        <div className="grid gap-3">
          {spotlightStats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-[1.4rem] border border-border bg-black/15 p-4 backdrop-blur"
            >
