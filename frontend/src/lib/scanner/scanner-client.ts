...
export async function listScannerEvents() {
  return apiFetch<ScannerEventSummary[]>("/api/events", undefined, {
    sort: "asc",
  });
}

export async function getScannerManifest(
  eventId: string,
  accessToken: string,
) {
...
export async function validateScannerTicket(
  ...
  return apiFetch<ScannerValidationResponse>(
