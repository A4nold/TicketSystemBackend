...
export type OrganizerEventSummary = {
  ...
  startsAt: string;
  status: string;
  ticketTypes: OrganizerEventResponse["ticketTypes"];
  timezone: string;
  title: string;
  venueAddress: string | null;
  venueName: string | null;
...
export type OrganizerTicketTypeResponse = OrganizerEventResponse["ticketTypes"][number];
...
export async function listOrganizerEvents() {
  return apiFetch<OrganizerEventSummary[]>("/api/events", undefined, {
    sort: "desc",
  });
}

export async function getOrganizerEventBySlug(slug: string) {
...
export async function acceptOrganizerStaffInvite(
  eventId: string,
  accessToken: string,
) {
